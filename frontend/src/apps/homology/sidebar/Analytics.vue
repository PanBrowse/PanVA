...
import SequenceFiltersModal from './SequenceFiltersModal.vue'
...
export default {
  ...
  computed: {
    ...
    metadataOptions(): DefaultOptionType[] {
      return sortBy(
        this.config.homology.sequenceMetadata.map(({ column, label }) => ({
          value: column,
          label,
        })),
        'label'
    ...
    filterPositionOptions(): DefaultOptionType[] {
      ...
      const additional: DefaultOptionType[] = []
      this.config.homology.variableMetadata.forEach(
        ({ column, label, type }) => {
          if (type === 'boolean') {
...
<template>
  <SidebarItem title="Analytics">
    ...
    >
      ...
      <AFormItem
        label="Annotation ref"
        v-if="config.homology.annotations.length !== 0"
      >
        <ASelect
          showSearch
          :dropdownMatchSelectWidth="false"
